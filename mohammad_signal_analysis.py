...
class Signal_Analysis_Mohammad:
    ...
    def get_signal(self):
        ...
        video_names = [os.path.splitext(os.path.basename(i))[0] for i in [self.file_name]]
        self.patient_title = video_names[0]
    ...
    def get_fps(self):
        ...
        for i in range(len(video_list)):
            ...
            video_dict[Path(video_list[i]).stem] = video_list[i]
        print(video_dict)
        print(self.patient_title)
    ...
    def phenotypes(self):
        ...
        ''' Speed plot '''
        color_list = ['red', 'orange', 'gold', 'green', 'k']
        color_list = ['k']
        # color_list = ['k']
        for clr in color_list:
            plt.figure(figsize=(8, 5))
            # color_assigned = 'green'
            plt.plot(self.cali_list[20:800], self.signal_filtered_derivative[20:800]*2.05,
                     label='original signal', linewidth=2.5, color=clr)
            # plt.xlabel('Time (sec)', fontsize = 22, weight='bold')
            plt.ylabel('Angular Velocity\n(deg/sec)', fontsize = 22, weight='bold')
            plt.grid(False)
            ...
            # plt.yticks([])
            plt.tight_layout()
            # plt.savefig(rf'/home/mohammad/Desktop/Tele_CF/scaled_figures/{self.patient_title}_{clr}.png', format='PNG', dpi=400)
    ...
    def plot_signal(self):
        ...
        # plt.xlim([0, 1600])
        # plt.title(self.patient_title)
        # plt.title('Angle Signal', fontsize=15, weight='bold')
        plt.xlabel('Time', fontsize=15, weight='bold')
        plt.ylabel('Angle (Degrees$^\circ$)', fontsize=15, weight='bold')
        # plt.plot(self.cali_list, self.signal, label='original signal')
        plt.plot(self.cali_list, self.signal, linewidth=2.5)

        # Modify x-tick font size and weight
...
def frailty_postprocess(excel_file, frailty_index_file, task_type, video_file_location, angular_acc):
    ...
    final_path = os.path.join(frailty_index_file, file_name)
    check_FI(FI_path=final_path)
    return final_path
